...
/// <summary>
/// Represents a lazily initialized implementation of <see cref="ThreadStorageSymbol"/> that is read from a PDB image.
/// </summary>
public class SerializedThreadStorageSymbol : ThreadStorageSymbol
{
    ...
    public SerializedThreadStorageSymbol(PdbReaderContext context, BinaryStreamReader reader, bool isGlobal)
    {
        _context = context;
    ...
    protected override CodeViewTypeRecord? GetVariableType()
    {
        return _context.ParentImage.TryGetLeafRecord(_typeIndex, out CodeViewTypeRecord? type)
            ...
            : _context.Parameters.ErrorListener.BadImageAndReturn<CodeViewTypeRecord>(
                $"Thread storage symbol contains an invalid type index {_typeIndex:X8}.");
    }
}
